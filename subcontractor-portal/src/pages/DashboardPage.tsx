...
const DashboardPage = () => {
  ...
  // NBFC Quotes
  const [myCwcrfs, setMyCwcrfs] = useState<any[]>([]);
  ...
  const fetchMyCwcrfs = async () => {
    try {
      const res = await cwcrfApi.getMyCwcrfs();
      setMyCwcrfs(res.data.cwcrfs || []);
    } catch {
      // silent - not critical for dashboard load
    }
  };
  ...
  const cwcrfsWithQuotes = myCwcrfs.filter(
    (c: any) =>
      c.nbfcQuotations &&
      c.nbfcQuotations.some((q: any) => q.status === "QUOTED"),
  );
  ...
  const tabs = [
    ...
    { id: "cwcaf", label: "CWC AF", icon: ClipboardList },
    {
      ...
      label: "NBFC Quotes",
      icon: Banknote,
      badge: cwcrfsWithQuotes.length > 0 ? cwcrfsWithQuotes.length : undefined,
    },
    {
      id: "additional-docs",
      label: "Extra Docs",
      icon: FolderUp,
      badge: (() => {
        const n = sc?.additionalDocuments?.filter(
          (d: any) => d.status === "REQUESTED",
        ).length;
        ...
        <AnimatePresence mode="wait">
                ...
                <div className="grid sm:grid-cols-4 gap-3">
                  {[
                    ...
                    },
                    {
                      id: "nbfc-quotes",
                      label: "NBFC Quotes",
                      desc: `${cwcrfsWithQuotes.length} with bids`,
                      icon: Banknote,
                      color: "violet",
                    },
                    {
                      id: "bids",
                      label: "View Bids",
                      desc: `${pendingBids.length} pending`,
                      icon: Gavel,
                      color: "amber",
                    },
                  ].map((action) => {
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-0.5">
                        ...
                        Funding request &amp; Interest preference
                      </p>
                    </div>
                  </div>
                  <Button
                    onClick={() => navigate("/cwcrf")}
                    className="shrink-0"
                  >
            ...
            >
              <Card className="border border-gray-200 shadow-none">
                <CardHeader>
                  ...
                  <CardDescription>
                    Track your financing case progress
                  </CardDescription>
                </CardHeader>
                <CardContent className="p-0">
              ...
              {dashboard?.bids?.length > 0 ? (
                ...
                <Card className="border border-gray-200 shadow-none">
                  <div className="flex flex-col items-center justify-center py-14 text-center px-6">
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      Bids from companies will appear here
                    </p>
                  </div>
                </Card>
              )}
          ...
          {activeTab === "nbfc-quotes" && (
            ...
            >
              ...
              {cwcrfsWithQuotes.length > 0 ? (
                cwcrfsWithQuotes.map((cwcrf: any) => {
                  const quotedBids =
                  ...
                  return (
                    ...
                    >
                      <CardContent className="p-5">
                        ...
                        {cwcrf.selectedNbfc?.nbfcId && (
                          <div className="bg-green-50 border border-green-200 rounded-lg p-3 flex items-center gap-3 mb-3">
                        ...
                        {!cwcrf.selectedNbfc?.nbfcId && (
                          <div className="space-y-2">
                            {quotedBids.map((quote: any, qIdx: number) => (
                              <div
                                key={qIdx}
                                className="border border-gray-200 rounded-lg p-3 hover:border-blue-300 hover:bg-blue-50/50 transition-colors"
                              >
                                <div className="flex items-start justify-between">
                                  <div className="flex-1">
                            ...
                            {pendingCount > 0 && (
                              <div className="border border-dashed border-gray-300 rounded-lg p-3 flex items-center justify-center">
                                ...
                                <p className="text-xs text-gray-400">
                                  {pendingCount} NBFC(s) yet to respond
                                </p>
                              </div>
                            )}
                          </div>
              ...
              <div className="flex items-start gap-3 bg-violet-50 border border-violet-200 rounded-xl p-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-violet-700 mt-0.5">
                    The operations team may request additional supporting
                    documents during verification. Upload each requested
                    document below.
                  </p>
                </div>
              </div>
              ...
              {sc?.additionalDocuments?.length > 0 ? (
                              ...
                              {isActive && additionalDocFile && (
                                <div className="flex justify-end">
                                  <Button
                                    ...
                                    onClick={() =>
                                      handleUploadAdditionalDoc(doc._id)
                                    }
                                  >
                                    {uploadingDocId === doc._id
                                      ? "Uploading..."
                ...
                <Card className="border border-gray-200 shadow-none">
                  <div className="flex flex-col items-center justify-center py-14 text-center px-6">
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      The operations team will notify you here if they need more
                      documents
                    </p>
                  </div>
                </Card>
                ...
                <CardContent>
                  <form className="space-y-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Textarea
                        id="cwcaf-remarks"
                        placeholder="Any additional information for the agreement..."
