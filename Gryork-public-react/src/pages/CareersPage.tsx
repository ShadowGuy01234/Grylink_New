  ...
  return (
    <SectionShell variant="light">
      <div className="mx-auto max-w-6xl px-4">
        <AnimatedReveal>
          <p className="badge-info mb-4">Talent & Growth</p>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {culture.map((item) => (
            <SignatureCard key={item} variant="proof" className="p-5 text-slate-700">
              {item}
            </SignatureCard>
          ))}
        </div>
        <div className="mt-8 grid gap-4">
          {CAREER_ROLES.map((role) => (
            <SignatureCard key={role.slug} variant="story" className="p-6">
