  ...
  async (req, res) => {
    try {
      ...
      if (!counterOffer || !counterOffer.amount) {
        return res
          .status(400)
          .json({ error: "counterOffer with amount is required" });
      }

      const bid = await bidService.negotiate(
        req.params.id,
        req.user._id,
        counterOffer,
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      const result = await bidService.lockCommercial(
        req.params.id,
        req.user._id,
      );
      res.json(result);
