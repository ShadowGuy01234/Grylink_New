...
const agentSchema = new mongoose.Schema(
  {
    ...
    commissions: [{
      ...
      epcName: { type: String },
      firstCwcId: { type: mongoose.Schema.Types.ObjectId, ref: 'Transaction' },
    ...
    misconductHistory: [{
      type: {
        type: String,
        enum: ['WEAK_EPC_PUSH', 'MISREPRESENTATION', 'FORCING_RISKY_DEALS', 'OTHER'],
      },
...
agentSchema.methods.recordCommission = async function(epcId, transactionId, amount, percentage) {
  ...
  const epcEntry = this.introducedEpcs.find(e => e.epcId.toString() === epcId.toString());
  if (epcEntry) {
